...
/************************************/
/* Like RAPARM-ms from revision 138 */
/* But ignores multimap reads       */
/************************************/
...
int main(int argc, char* argv[]) {
	...
	std::cerr << std::endl << "    _|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|";
	std::cerr << std::endl << "    _|                                                            _|"; 
	std::cerr << std::endl << "    _|              _|_|    _|      _|                            _|";
	std::cerr << std::endl << "    _|            _|    _|  _|_|  _|_|                            _| unique";
	std::cerr << std::endl << "    _|  _|    _|  _|_|_|    _|  _|  _|        _|_| _|_|     _|_|  _| Read";
	std::cerr << std::endl << "    _|  _|    _|  _|   _|   _|      _|  _|_|  _|  _|  _|    _|    _| Mapping over";
	std::cerr << std::endl << "    _|    _|_|    _|    _|  _|      _|        _|      _|  _|_| 64 _| Multiple";
	std::cerr << std::endl << "    _|                                                            _| Splicesites"; 
	std::cerr << std::endl << "    _|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_|_| 64-bit" << std::endl << std::endl;
	...
	int map_c = 0, kill_c = 0, droppedreads = 0;
	for(int r=0;r < readSize; ++r) {
		if (reads[r] == NULL) continue;
		if (r % 1000 == 0) {
			time(&currentTime);
		...
		kill_c += reads[r]->unify(bestMatch);
		// remove reads with too many mappings
		if (reads[r]->countMappings() > 1) {
			//delete reads[r];
			reads[r] = NULL;
			++droppedreads;
		}
	...
	std::cerr << "\r " << (int)finaltime << " seconds elapsed  (" << kill_c << " mappings out of " << map_c << " were removed, " << droppedreads << " ambiguous reads were dropped)" << std::endl;
	...
	/***************************/
	if (newExpressedRetrocopies > 0 && (extendRetros)) std::cerr << "Retros cannot be detected using only unique read mappings";
	...
	//RAPARM ALGO
	std::cerr << std::endl << "Checking uniqueness..." << std::endl;
	time(&startTime);
	for (int r = 0; r < readSize; r++) {
		...
		if (reads[r] == NULL || reads[r]->isUnambiguous()) continue;
			...
			/* do nothing */
			std::cerr << "OOPS: Found an ambiguous read -> DEBUG STOP";
			abort();
		}
		if (r % 1000 == 0) {
			time(&currentTime);
